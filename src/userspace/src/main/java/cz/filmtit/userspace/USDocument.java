 ...
 * of the class are used keep the wrapped document properties persistent by the database mapping. There are also
 * further properties which are used exclusively by the User Space.
 * @author Jindřich Libovický
 */
public class USDocument extends DatabaseObject {
    /**
     * ID of the owner of the document. (It is only here, not in the shared object.)
     */
    ...
    /**
     * A map of translation results this documents consists of. It is insured in the code it is always a synchronized
     * map. The collection is mirrored in the wrapped documents by the objects of shared translation results classes.
     */
    private SortedMap<ChunkIndex, USTranslationResult> translationResults;

    /**
     * Flag is the the user deleted the document. It is kept in the database until the feedback is provided to the core,
     ...
     * the media source is not set) and saves the new document immediately to the database
    ...
    /**
     * Gets the path of the video on the user's local machine which was used last time the client run.
     * It is private because it is used by Hibernate only.
     * @return
     */
    ...
    /**
     * Sets the path to the video on the user's local machine. It is private beacause it is use by Hibernate only.
    ...
    /**
     * Gets the database id of the owner of the document. It is private and used by Hibernate only.
     * @return
     */
    ...
    /**
     * Sets the database id of user owning the document. It is private and used by Hibernate only.
    ...
    /**
     * Sets the source language of the document. Only the language code is passed to the method, corresponding
     * language object is created in the wrapped document object.
    ...
    /**
     * Gets the media source object representing the movie or TV show of the document.
    ...
    /**
     * Sets the media source of the document. It is private because it is used by Hibernate only. There is many-to-one
    ...
    /**
     * Gets the set of indexes (unique within a document) of the translation results.
    ...
    /**
     * Gets the time of the last change of the document (from the wrapped document).
    ...
    /**
     * Gets the total number of chunks in the document. (It is in a separate variable in order to be get the number
     * even if the translation results are not loaded from the database.)
     * @return
     */
    ...
    /**
     * Sets the total number of chunks (calls the wrapped shared object). It is private and used by Hibernate only.
    ...
    /**
     * Gets the number of translation results where the user translation has been already set. (Calls the wrapped share object.)
    ...
    /**
     * Sets the count of already translated chunks. It is called both from Hibernate and from the setUserTranslation
     * method in the Session class which is called when the user sets or changes the translation.
    ...
    /**
     * Gets the flag if a user marked the document to be deleted from the database
