...
\todo{who knows it, please add the browser support both here and to the user manual}
...
\todo{I already wrote a lot into the implementation process and I dont want to have duplicities in the documentation, so I am now keeping the development description in implementation process but moving detailed descriptions of the final solution from implementation process into this section. Prosim bud pacient :-)}
...
\todo{maybe describe in more detail, maybe not \\ I would delete it (Jindřich)}
...
The {\tt Storable} interface is currently only implemented by the {\tt SetUserTranslation} class, for reasons described in the Implementation Process \todo{ref}.
...
We intentionally did not implement very sophisticated controls for the player (since it would make the design too complicated). All the player can do is replay the whole current window or pause the playing\todo{I will maybe implement the unpausing if I got time. If I don't have time, I won't.}.
