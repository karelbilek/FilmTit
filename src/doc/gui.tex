...
Also, call parameters are not listed if they are either already specified in REF sd: translating a document or only of a technical nature.



Before using the Local Storage, it must always be first checked whether it is supported -- if it is not, Offline Mode is not offered to the user and a relevant message is displayed.
