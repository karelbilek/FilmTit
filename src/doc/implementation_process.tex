...
Created a Storable interface for classes that implement serialization and deserialization methods.
Currently only implemented by one class (SetUserTranslation), but is desinged to be easily extendable if we decide to store other data in Local Storage as well. (Logically separated into LocalStorageHandler, Storable interface and the SetUserTranslation class).
