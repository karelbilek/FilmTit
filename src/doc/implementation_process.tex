...
When implementing the feature of exporting the subtitles, we encountered an unexpected issue:
GWT cannot create a file, neither on server (beacause it is all Javascript) nor on the user's computer (because of security restrictions).
...
\begin{itemize}
\item not really what a user expects
\end{itemize}
...
\item should handle access rights in Jetty
\end{itemize}

\item create a new servlet (using JSP) for downloading the file
\begin{itemize}
\item we were afraid that it would be too much extra work
\item it turned out not to be so difficult so we used that
\end{itemize}
