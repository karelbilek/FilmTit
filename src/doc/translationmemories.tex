...
Subtitles on DVDs are stored in bitmap format within the movie itself.\footnote{Although official DVD documentation is not available online for free, you can see some information here - \url{http://dvd.sourceforge.net/spu_notes}} With the birth of unofficial movie file sharing, those bitmaps became too big to distribute, so several unofficial formats, based on text, were made.

In those cases, the subtitles -- now in text format instead of bitmap, as in DVD -- are in a standalone file which contains the actual subtitles and some meta data telling the media players which subtitle chunks should be displayed at the which time interval of the video playback.
...
MicroDVD \emph{sub} format is a format, used by free, but proprietary MicroDVD media player, which is not developed from 2001. This format also lacks any formal specification and support is usually worse among players.
...
For our purposes, we can take note of one important thing, partly visible in the example on top: not every subtitle ``chunk'' (that is, lines displayed on screen at the same time) is a sentence -- sometimes, there are more sentences in one ``chunk'', sometimes, there is one sentence split across many ``chunks''. We describe how we dealt with this problem in the section \todo{???}???????.
