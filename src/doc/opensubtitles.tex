...
On the other hand, the reason for \emph{not} keeping only the splitting to subtitle items and for splitting into more smaller chunks is, that the splitting into subtitle items seems sometimes too arbitrary and we would miss the shortest sentences, which are the easiest ones to find in a translation memory.
...
After this step, we almost have the final corpus. What we have to do before saving it into a final corpus is splitting the aligned items into chunks, as described in Section~\ref{os:sentence_splitting}. If both Czech and English have the same number of chunks, we split them and add those to the corpus; if the number is different, we ignore them.
...
\end{center}
\caption{An illustration for the distance-based alignment. Circle represents subtitle, red line represent chosen alignment, black ones are some candidates, red circles are the subtitles that are aligned}\label{distance_align}
\end{figure}
