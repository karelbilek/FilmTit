...
with perfect matches and 3.1\,\% of total mismatches. \todo{I have \textbf{no clue whatsoever} what the percentages mean. Is it the editing distance?}The scores for partial matches are captured in table \ref{opensubtitles:matchTable}.
...
After looking at some randomly selected files we have decided to use just movies for which we have a pair of files with at least \todo{Again, what does the 70\% mean??}70 \% match. Again, lower match score doesn't mean lower quality of translation, but rather how the subtitles match to each other. 
...
With some consideration, we count precision \todo{this looks like shit}as equal to $\frac{\text{fully true positive}}{\text{false positive}+\text{fully true positive}+\text{partly true positive}}$ and recall as $\frac{\text{fully true positive}}{\text{fully true positive}+\text{partly true positive}+\text{false negative}}$.
...
With these definitions, precision and recall are as follows (\todo{sounds terrible}the numbers are percentages):
...
What also has to be noted here that the result counts are not evenly distributed -- on the more "popular" sentences like "I don't know", we might have hundreds of matches; we recieve only first 30 ones. This will, of course, skew the precision a bit (since we take only 30 chunks as received); however, it emulates the working of translation memory anyway.
