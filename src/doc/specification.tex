...
\todo{I am not sure if this whole section should be the same thing as we handed in or if it could be rewritten}

The application has two parts -- \todo{Unclear (even to me) -- is the server part one of the parts? or how is it meant exactly}the server part consisting of  two components
\begin{enumerate}
...
The user interface provides the user a practical and intuitive environment for the translation and \todo{technically speaking, we are communicating with userspace, not the TM} communicates with the translation memory.
From the user's view, the main functionality is parsing the subtitle into individual \todo{shouldn't  we define what a "chunk" is? :)}chunks 
...
The interface \todo{not right now it doesn't} provides complex tool for the subtitle editing including the possibility to adjust the timing, add new chunks or delete some. 
...
The server part queries the translation memory for similar sentences which have occurred in the subtitles before and has been already translated. Different method of retrieving the similar chunks are \todo{?????} used ... For evaluating how much the ...
