...
For full-text search, the full-text search integrated in Postgres is used (this used to be \emph{TSearch2}). By default, a  number of languages are available. If one wants to use additional languages, they must be installed manually. For the Czech language, we prepared an installation script which can run using following command (example for running the command on an Ubuntu Linux system).
