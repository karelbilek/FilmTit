...
Filmtit uses free mailserver and account from Gmail for sending emails. Administrator can change mailer server, account and text of messages too. 
Userspace part properties contains whole configuration of mailer. Configuration can be divided into two parts. First part consistis of entries with values, which are needed to establishing connection
\begin{itemize}
...
\end{itemize}
Second part contains entries for setting messages. Administrator can change subject and text message. Message contains strings starts and ends with '\%' - templates. These templates are replaced by data from database.
\begin{itemize}
