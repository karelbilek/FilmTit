...
Moses server can be running on either separate computer or the same computer, as the FilmTit system. You connect to Moses server via HTTP and sent it sentences to translate as remote procedure calls; however, there is no kind of authentification, so you need to secure your Moses server for example by your network infrastructure or risk unauthorized translations request.

Moses is an experimental software and it's obvious from the slightly complicated installation; we will present an easy way to install Moses on operating systems with APT package system (ubuntu, debian) and a way to run Moses with our models, which are on \todo{who knows}the accompanying portable media; if you have a different operating system, you \todo{write it better}will have to install according to the Moses website.

Except for APT, you will also need git and GCC. You will also need \texttt{sudo} privileges.

\subsection{Installing Moses server}
\begin{enumerate}
    \item
First, you need to checkout Moses from their git repository.
...
\item
Second, you need to install \texttt{boost} libraries.
...
\item
You also need to install xml rpc libraries.
...
\item
Now you can finally build the moses server. If you have for example 4 cores, you can build moses by following.
...
If you have boost at nonstandard location, you will have to specify it with the \texttt{-{}-with-boost=/path/to/boost} switch.
...
For running Moses server, you will need our folder with data models. \todo{Who knows}This data should be supplied with the something something.

Now, let's suppose our data with models are at folder \texttt{/data/} and mosesdecoder is at folder \texttt{/bin/}. Then, you can easily start the server at port \texttt{8081} with 4 threads (recommended number of threads is number of cores) by running
...
and let the Moses server start. Nothing more is needed.
