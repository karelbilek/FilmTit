...
Moses is an experimental software and it is obvious from the slightly complicated installation; we will present an easy way to install Moses on an operating systems with the APT package system (ubuntu, debian) and a way to run Moses with our models, which are on \todo{WHERE??}the accompanying portable media; on different operating systems, the steps should be similar, but the official Moses website\footnote{\url{http://statmt.org/moses}} should be consulted for further information.
